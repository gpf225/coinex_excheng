...
static void depth_reply(nw_ses *ses, json_t *result)
{
    json_t *new_result = json_object();
...
static int depth_sub_reply(const char *market, const char *interval, json_t *result)
{
    ...
    while ((entry = dict_next(iter)) != NULL) {
        ...
        notify_message(ses, CMD_CACHE_DEPTH_UPDATE, result);
    }
...
static void on_timer(nw_timer *timer, void *privdata) 
{
    ...
    while ((entry = dict_next(iter)) != NULL) {
        ...
        struct dict_cache_val *cache_val = get_cache(cache_key, settings.sub_depth_interval * 100);
        if (cache_val != NULL) {
            depth_sub_reply(key->market, key->interval, cache_val->result);
        } else {
