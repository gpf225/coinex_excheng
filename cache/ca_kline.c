...
static dict_t *dict_kline_sub;
static nw_timer timer;

struct dict_kline_key {
    char market[MARKET_NAME_MAX_LEN];
    int interval;
};

struct dict_kline_sub_val {
    dict_t *sessions; 
};
...
static uint32_t dict_kline_sub_hash_function(const void *key)
{
    return dict_generic_hash_function(key, sizeof(struct dict_kline_key));
}

static int dict_kline_sub_key_compare(const void *key1, const void *key2)
{
    return memcmp(key1, key2, sizeof(struct dict_kline_key));
}

static void *dict_kline_sub_key_dup(const void *key)
{
    struct dict_kline_key *obj = malloc(sizeof(struct dict_kline_key));
    memcpy(obj, key, sizeof(struct dict_kline_key));
    return obj;
}

static void dict_kline_sub_key_free(void *key)
{
    free(key);
}

static void *dict_kline_sub_val_dup(const void *val)
{
    struct dict_kline_sub_val *obj = malloc(sizeof(struct dict_kline_sub_val));
    memcpy(obj, val, sizeof(struct dict_kline_sub_val));
    return obj;
}

static void dict_kline_sub_val_free(void *key)
{
    struct dict_kline_sub_val *obj = key;
    if (obj->sessions != NULL)
        dict_release(obj->sessions);
    free(obj);   
}

static dict_t* dict_create_kline_session(void)
{
    dict_types dt;
...
static void on_timeout(nw_state_entry *entry)
{
    ...
    if (state->ses != NULL) {
        if (state->ses->id == state->ses_id) {
            ...
            reply_rpc.command = CMD_MARKET_KLINE;
...
static void kline_reply(nw_ses *ses, json_t *result, uint32_t sequence)
{
    json_t *new_result = json_object();
    ...
    rpc_pkg reply_rpc;
    memset(&reply_rpc, 0, sizeof(reply_rpc));
    reply_rpc.command = CMD_CACHE_KLINE;
...
static int kline_sub_reply(const char *market, int interval, json_t *result)
{
    ...
    dict_entry *entry = dict_find(dict_kline_sub, &key);
    if (entry == NULL)
        return -__LINE__;

    struct dict_kline_sub_val *obj = entry->val;
...
static void on_backend_recv_pkg(nw_ses *ses, rpc_pkg *pkg)
{
    ...
    case CMD_MARKET_KLINE:
        if (state->ses) { // out request
            if (state->ses->id == state->ses_id) {
                kline_reply(state->ses, reply, state->sequence);
            ...
            if (!is_error)
                kline_sub_reply(state->market, state->interval, result);
        }
...
int kline_request(nw_ses *ses, rpc_pkg *pkg, const char *market, time_t start, time_t end, int interval)
{
    ...
    json_t *params = json_array();
    json_array_append_new(params, json_string(market));
    json_array_append_new(params, json_integer(start));
    json_array_append_new(params, json_integer(end));
    json_array_append_new(params, json_integer(interval));
    ...
    req_pkg.command   = CMD_MARKET_KLINE;
...
{
    log_info("kline subscribe, market: %s, interval: %d", market, interval);
    struct dict_kline_key key;
    memset(&key, 0, sizeof(key));
    strncpy(key.market, market, MARKET_NAME_MAX_LEN - 1);
    key.interval = interval;
    ...
    dict_entry *entry = dict_find(dict_kline_sub, &key);
    if (entry == NULL) {
        struct dict_kline_sub_val val;
        memset(&val, 0, sizeof(val));

        val.sessions = dict_create_kline_session();
        if (val.sessions == NULL)
            return -__LINE__;

        entry = dict_add(dict_kline_sub, &key, &val);
        if (entry == NULL) {
...
int init_kline(void)
{
    ...
    dict_types dt;
    memset(&dt, 0, sizeof(dt));
    dt.hash_function   = dict_kline_sub_hash_function;
    dt.key_compare     = dict_kline_sub_key_compare;
    dt.key_dup         = dict_kline_sub_key_dup;
    dt.key_destructor  = dict_kline_sub_key_free;
    dt.val_dup         = dict_kline_sub_val_dup;
    dt.val_destructor  = dict_kline_sub_val_free;

    dict_kline_sub = dict_create(&dt, 512);
    if (dict_kline_sub == NULL) {
        log_stderr("dict_create failed");
        return -__LINE__;
    }

    nw_timer_set(&timer, settings.sub_kline_interval, true, on_timer, NULL);
