...
{
    sds sql = sdsempty();
    sql = sdscatprintf(sql, "SELECT `user_id` from user WHERE `user_id` > '%u' ORDER BY `user_id` ASC LIMIT %u", last_user_id, MAX_USER_SIZE);
    ...
    if (user_list->size == 0) {
        log_info("no more users, last user_id:%u", last_user_id);
    ...
    last_user_id = user_list->users[user_list->size - 1];
    log_info("fetch %u users, first user_id:%u last_user_id:%u", user_list->size, user_list->users[0], last_user_id);
