...
static int process_orders_message(json_t *msg)
{
    ...
    const char *fee_available = json_string_value(json_object_get(balance, "fee_available"));
    const char *fee_frozen = json_string_value(json_object_get(balance, "fee_frozen"));

    if (user_id == 0 || stock == NULL || money == NULL || stock_available == NULL || stock_frozen == NULL || money_available == NULL || money_frozen == NULL)
        return -__LINE__;
    if (fee_asset && (!fee_available || !fee_frozen))
        return -__LINE__;
    ...
    if (fee_asset && strcmp(fee_asset, stock) != 0 && strcmp(fee_asset, money) != 0) {
        asset_on_update(user_id, account, fee_asset, fee_available, fee_frozen);
        if (account == 0) {
