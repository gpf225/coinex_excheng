...
int init_message(void)
{
    ...
    kafka_deals = consumer_create(settings.brokers, TOPIC_DEAL, deals_offset + 1, on_deals_message);
    if (kafka_deals == NULL)
        return -__LINE__;
    ...
    kafka_orders = consumer_create(settings.brokers, TOPIC_ORDER, orders_offset + 1, on_orders_message);
