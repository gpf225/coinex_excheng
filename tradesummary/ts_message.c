...
static void on_deals_message(sds message, int64_t offset)
{
    ...
    time_t time_minute = ((time_t)timestamp) / 60 * 60 - 60;
    if (time_minute > last_dump_time) {
        int ret = dump_users_detail();
        if (ret < 0) {
            log_error("dump_users_detail fail ret: %d", ret);
        }
    }
